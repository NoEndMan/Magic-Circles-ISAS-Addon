...
### Changed
- Circles are now centered to the client cursor and to the caster head by default. 
...
in order to achieve a general shader compatibility. This entities are never saved to the level and only viewable 
to the client.
...
- Fixed style change using config doesn't always work.
